...
var finishedWork = false, gotTickets = true
...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    ...
    if bugs > 10 {
        print("Too many bugs to release the game")
    }
    
    if !music {
        print("Add music before release the game")
    }
    
    if levels < 5 {
        print("Add more levels before release the game")
    }
}
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    if bike && run {
        print("\(name) is physically prepared for a duathlon traning.")
    } else if !bike && !run {
